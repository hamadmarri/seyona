angular.module('phonertcdemo')
  ...
  .controller('CountryCallCtrl', function ($scope, $state, $rootScope, $timeout, $interval,
         $ionicModal, $stateParams, signalingCountry, CountryService, ContactsServiceForCountry) {
    ...
    function call(isInitiator, contactName) {
      ...
      session.on('sendMessage', function (data) { 
        signalingCountry.emit('sendMessage', contactName, { 
      ...
      session.on('disconnect', function () {
        ...
        if (Object.keys($scope.contacts).length === 0) {
          signalingCountry.emit('sendMessage', contactName, { type: 'ignore' });
        ...
        signalingCountry.emit('incrementCallsCount');
    ...
    if ($scope.isCalling) {
      ...
      signalingCountry.emit('sendMessage', $stateParams.contactName, { type: 'countrycall', callingCountryPerson: $scope.myCountry });
    ...
    $scope.ignore = function () {
      ...
      if (contactNames.length > 0) { 
        ...
        signalingCountry.emit('sendMessage', $stateParams.contactName, { type: 'ignore' });
    ...
    $scope.answer = function () {
      ...
      setTimeout(function () {
        ...
        signalingCountry.emit('sendMessage', $stateParams.contactName, { type: 'answer' });
      }, 1500);
    };
    ...
    $scope.addContact = function (newContact) {
      ...
      signalingCountry.emit('sendMessage', newContact, { type: 'countrycall' });
      ...
      switch (message.type) {
        case 'answer':
          ...
          if (existingContacts.length !== 0) {
            signalingCountry.emit('sendMessage', name, {
              type: 'add_to_group',
        ...
        //       $timeout(function () {
        //         signalingCountry.emit('sendMessage', contact, { 
        //           type: 'add_to_group',
    ...
    signalingCountry.on('messageReceived', onMessageReceive);

    $scope.$on('$destroy', function() { 
      signalingCountry.removeListener('messageReceived', onMessageReceive);
    });
